...
const FONTS = ["Calibri", "Georgia", "Times New Roman", "Arial", "Courier New", "Verdana", "Trebuchet MS"];
...
function Editor({ fileId, fileName, onClose }: EditorProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "sessions" | "write">("write");
  const [fileData, setFileData] = useState<FileData>(() => getFileData(fileId, fileName));
  ...
  const [customColor, setCustomColor] = useState(DEFAULT_FORMATTING.customColor);
  ...
  useEffect(() => {
    const fetchAndInitDocument = async () => {
      try {
        const response = await api.get<DocumentDetail>(`/api/documents/${fileId}`);
  ...
  const handleSaveSession = useCallback(async () => {
    ...
    alert("Session saved!");
  }, [fileData, fileId, edits, pastes, pauses, font, fontSize, textColor, bgColor, customColor, customBg]);
  ...
  const avgWpm =
    totalSessions > 0
      ? Math.round(fileData.sessions.reduce((a, s) => a + s.wpm, 0) / totalSessions)
      : 0;
    ...
    <div className={styles.root}>
        ...
        <div className={styles.writeContainer}>
          ...
          <div className={styles.editorCard}>
            ...
            <div className={styles.toolbar}>
              ...
              {/* Format buttons */}
              <ToolBtn label="B" bold onClick={() => exec("bold")} title="Bold" />
              <ToolBtn label="I" italic onClick={() => exec("italic")} title="Italic" />
              <ToolBtn label="U" underline onClick={() => exec("underline")} title="Underline" />
              <ToolBtn label="S" strike onClick={() => exec("strikeThrough")} title="Strikethrough" />

              <div className={styles.toolbarSep} />

              {/* Headings */}
              <ToolBtn label="H1" onClick={() => applyHeading("h1")} title="Heading 1" />
              <ToolBtn label="H2" onClick={() => applyHeading("h2")} title="Heading 2" />
              <ToolBtn label="H3" onClick={() => applyHeading("h3")} title="Heading 3" />
              <ToolBtn label="¶" onClick={() => applyHeading("p")} title="Paragraph" />

              <div className={styles.toolbarSep} />

              {/* Align */}
              <ToolBtn label="≡" onClick={() => exec("justifyLeft")} title="Align Left" />
              <ToolBtn label="≡" onClick={() => exec("justifyCenter")} title="Center" style={{ letterSpacing: 1 }} />
              <ToolBtn label="≡" onClick={() => exec("justifyRight")} title="Align Right" />
              <ToolBtn label="≡" onClick={() => exec("justifyFull")} title="Justify" />
            </div>
            ...
            <div className={`${styles.toolbar}`} style={{ paddingTop: 0 }}>
              ...
              {/* Custom colors */}
              <label className={styles.colorPickerWrap} title="Custom text color">
                <input
                  type="color"
                  value={customColor}
                ...
                />
                <div className={styles.colorSwatch} style={{ background: customColor }} />
              </label>

              <label className={styles.colorPickerWrap} title="Custom highlight color">
                <input
                  type="color"
                  value={customBg}
                  className={styles.colorInput}
                ...
                />
                <div className={styles.colorSwatch} style={{ background: customBg }} />
              </label>

              <div className={styles.toolbarSep} />

              {/* List / indent */}
              <ToolBtn label="• —" onClick={() => exec("insertUnorderedList")} title="Bullet List" />
              <ToolBtn label="1." onClick={() => exec("insertOrderedList")} title="Numbered List" />
              <ToolBtn label="→" onClick={() => exec("indent")} title="Indent" />
              <ToolBtn label="←" onClick={() => exec("outdent")} title="Outdent" />
            ...
            <div
              ...
              onPaste={handlePaste}
              onInput={() => {
                // trigger re-render for live stats
                setEdits((e) => e); // no-op to rerender; we read from ref
              }}
              spellCheck
            />
            ...
            <div className={styles.footer}>
              <div className={styles.footerStats}>
                <StatItem label="Words:" value={words} color="#6b7280" />
                <StatItem label="Chars:" value={chars} color="#6b7280" />
                <StatItem label="Edits:" value={edits} color="#d97706" />
                <StatItem label="Pastes:" value={pastes} color="#6b7280" />
              </div>
      ...
      {activeTab === "sessions" && (
        <div className={styles.tabContent}>
          <h2 className={styles.sectionTitle}>Writing Sessions — {fileData.name}</h2>
          {fileData.sessions.length === 0 ? (
            <p className={styles.emptyMsg}>No sessions saved yet. Write something and click "Save session".</p>
          ) : (
            <div className={styles.sessionList}>
              {[...fileData.sessions].reverse().map((s, i) => (
                <div key={s.id} className={styles.sessionCard}>
                  <div className={styles.sessionHeader}>
                    <span className={styles.sessionNum}>Session #{fileData.sessions.length - i}</span>
                  ...
                  <div className={styles.sessionStats}>
                    ...
                    <SessionStat label="Duration" value={`${Math.round(s.duration / 60)}m ${s.duration % 60}s`} />
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === "overview" && (
        <div className={styles.tabContent}>
          ...
          <div className={styles.overviewGrid}>
            <OverviewCard label="Total Sessions" value={totalSessions} icon="📝" />
            ...
            <OverviewCard
              ...
              icon="📅"
            />
            <OverviewCard label="File Name" value={fileData.name} icon="📄" />
          </div>
...
function ToolBtn({
  ...
  return (
    <button
      ...
      style={{ ...decorationStyle, ...extraStyle }}
      onClick={onClick}
      title={title}
    >
      {label}
...
function StatItem({
  ...
  return (
    <span style={{ color, fontSize: 13 }}>
      {label} <strong style={{ color: "#e5e7eb" }}>{value}</strong>
    </span>
  );
}
...
function SessionStat({
  ...
  return (
    <div className={styles.sessionStat}>
      <div className={styles.sessionStatVal} style={{ color: accent ? "#f59e0b" : "#e5e7eb" }}>
        {value}
      </div>
  ...
  return (
    <div className={styles.overviewCard}>
      ...
      <div
        ...
        style={{ color: accent ? "#f59e0b" : "#e5e7eb" }}
      >
        {value}
      </div>
      ...
      {sessions.map((s, i) => (
        <div key={s.id} className={styles.chartBarWrap}>
          <div
            ...
            style={{
              height: `${Math.max(4, (s.wpm / max) * 100)}%`,
            }}
            title={`Session ${i + 1}: ${s.wpm} WPM`}
          />
